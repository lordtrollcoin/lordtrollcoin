...
class P2PPolicyTests(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        # a LORDTROLLCOIN Core node that behaves similar to mainnet policies
    ...
    def run_test(self):
        ...
        ### test constants ###
        koMeme = Decimal("0.00000001")          # 1 KoMeme expressed in LTR
        ten = Decimal("10.0")                  # uniform 10 LTR seed moneys
        ...
        # create a bunch of UTXO with seed money from the LORDTROLLCOIN Core wallet
        for i in range(20):
        ...
        # test legacy output of 1 LTR output and 1 LTR fee
        ...
        output = { self.tgtAddr: ten - relay_fee_per_byte * 192 + koMeme }
        ...
        amount = soft_dust_limit - koMeme
        change = ten - amount - relay_fee_per_byte * 225 - soft_dust_limit + koMeme
        output = { self.tgtAddr : amount, self.srcAddr: change }
        self.run_relay_test(output, 66, 225)

        # test a 1 koMeme output with sufficient fee
        amount = koMeme
        change = ten - amount - relay_fee_per_byte * 225 - soft_dust_limit
        output = { self.tgtAddr : amount, self.srcAddr: change }
        self.run_relay_test(output, 64, 225) # 64 = dust

        # test a 1 koMeme output with insufficient fee
        amount = koMeme
        change = ten - amount - relay_fee_per_byte * 225 - soft_dust_limit + koMeme
        output = { self.tgtAddr : amount, self.srcAddr: change }
    ...
    # spend seed money with a key not in the LORDTROLLCOIN Core wallet.
    def spend_utxo(self, output, expected_size, retries=0):
        # construct the transaction using LORDTROLLCOIN Core raw tx APIs
        ...
        # and send the tx from there rather than from LORDTROLLCOIN Core, to test
