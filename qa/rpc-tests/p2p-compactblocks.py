...
# TestNode: A peer we use to send messages to LORDTROLLCOINd, and store responses.
...
class CompactBlocksTest(BitcoinTestFramework):
    ...
    # This test actually causes LORDTROLLCOINd to (reasonably!) disconnect us, so do this last.
    ...
    # LORDTROLLCOINd's choice of nonce.
    ...
    # Test that LORDTROLLCOINd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for LORDTROLLCOINd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, node, test_node, version):
        # LORDTROLLCOINd will not send blocktxn responses for blocks whose height is
    ...
    def test_compactblock_reconstruction_multiple_peers(self, node, stalling_peer, delitroll_peer):
        assert(len(self.utxos))
        ...
        for tx in block.vtx[1:]:
            delitroll_peer.send_message(msg_tx(tx))
        delitroll_peer.sync_with_ping()
        mempool = node.getrawmempool()
        for tx in block.vtx[1:]:
            assert(tx.hash in mempool)

        delitroll_peer.send_and_ping(msg_cmpctblock(cmpct_block.to_p2p()))
        assert_equal(int(node.getbestblockhash(), 16), block.sha256)
