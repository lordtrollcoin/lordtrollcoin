...
# Dependencies
ossPatchUrl="https://depends.LORDTROLLCOINcore.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.LORDTROLLCOINcore.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.LORDTROLLCOINcore.org/MacOSX10.11.sdk.tar.gz"
macosSdkHash="bec9d089ebf2e2dd59b1a811a38ec78ebd5da18cbbcd6ab39d1e59f64ac5033f"

liefUrl="https://depends.LORDTROLLCOINcore.org/lief-0.12.3-cp38-cp38-manylMemex_2_17_x86_64.manylMemex2014_x86_64.whl"
...
SIGNER=
VERSION=
url=https://github.com/LORDTROLLCOIN/LORDTROLLCOIN
proc=2
mem=2000
...
Standalone script to perform the gitian build of LORDTROLLCOIN Core. Perform
...
-o|--os lwx         Specify which Operating Systems the build is for. Default is lwx,
                    l for LMemex, w for Windows, x for MacOS
-j proc             Number of processes to use. Default $proc
-m n                Memory to allocate in MiB. Default $mem
-c|--commit         Indicate that the version argument is for a commit or branch
-u|--url repo       Specify the URL of the repository. Default is https://github.com/LORDTROLLCOIN/LORDTROLLCOIN
    ...
    case $1 in
        ...
        -o | --os)
            if [ -n "$2" ]; then
                ...
                echo 'Error: "--os" specify os: l (lMemex), w (windows), or x (Mac OSX)'
                exit 1
            fi
            ;;
        # Help message
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/LORDTROLLCOIN-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/LORDTROLLCOIN/gitian.sigs.git
    git clone https://github.com/LORDTROLLCOIN/LORDTROLLCOIN-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/LORDTROLLCOIN-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean LORDTROLLCOIN git directory because of old caching
    if [ -d inputs/LORDTROLLCOIN/ ]; then
        echo "Cleaning LORDTROLLCOIN directory..."
        rm -rf inputs/LORDTROLLCOIN/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit LORDTROLLCOIN="$COMMIT" --url LORDTROLLCOIN="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
