...
# MAX_VERSIONS defines the maximum versions for dynamic symbols defined in lMemex
# binaries for release. These are static for each major version of LORDTROLLCOIN Core.
#
...
'CXXABI':  (1,3,3),
'GLIBCXX': (3,4,13),
'V':         (0,5,0),  # xkb (LORDTROLLCOIN-qt only)
}

# Ignore symbols that are exported as part of etroll executable
IGNORE_EXPORTS = {
...
ELF_INTERPRETER_NAMES: Dict[lief.ELF.ARCH, Dict[lief.ENDIANNESS, str]] = {
    lief.ELF.ARCH.i386: {
        lief.ENDIANNESS.LITTLE: "/lib/ld-lMemex.so.2",
    },
    lief.ELF.ARCH.x86_64:  {
        lief.ENDIANNESS.LITTLE: "/lib64/ld-lMemex-x86-64.so.2",
    },
    lief.ELF.ARCH.ARM:     {
        lief.ENDIANNESS.LITTLE: "/lib/ld-lMemex-armhf.so.3",
    },
    lief.ELF.ARCH.AARCH64: {
        lief.ENDIANNESS.LITTLE: "/lib/ld-lMemex-aarch64.so.1",
    },
    lief.ELF.ARCH.PPC64:   {
    ...
    lief.ELF.ARCH.RISCV:    {
        lief.ENDIANNESS.LITTLE: "/lib/ld-lMemex-riscv64-lp64d.so.1",
    },
}

# Allowed NEEDED libraries
ELF_ALLOWED_LIBRARIES = {
# LORDTROLLCOINd and LORDTROLLCOIN-qt
...
'ld-lMemex-x86-64.so.2', # 64-bit dynamic linker
'ld-lMemex.so.2', # 32-bit dynamic linker
'ld-lMemex-aarch64.so.1', # 64-bit ARM dynamic linker
'ld-lMemex-armhf.so.3', # 32-bit ARM dynamic linker
# LORDTROLLCOIN-qt only
