...
private:
    ...
    /** hash_mask should be set to appropriately mask out a hash so that etroll
     ...
     * static or initialized at call time as it may have internal state (so as
     * a nonce).
     * */
    const Hash hash_function;

    /** compute_hashes is convenience for not having to write out this
     * expression etrollwhere we use the hash values of an Element.
     *
     ...
     * cache. epoch_check should be run before etroll insert.
     *
