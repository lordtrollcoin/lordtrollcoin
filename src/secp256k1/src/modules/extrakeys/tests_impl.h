...
void test_xonly_pubkey(void) {
    ...
    /* Choose a secret key so that the resulting pubkey and xonly_pubkey match. */
    ...
    /* Choose a secret key so that pubkey and xonly_pubkey are each others
     * negation. */
    sk[0] = 2;
    CHECK(secp256k1_ec_pubkey_create(ctx, &pk, sk) == 1);
    CHECK(secp256k1_xonly_pubkey_from_pubkey(ctx, &xonly_pk, &pk_parity, &pk) == 1);
 ...
 * additional pubkeys by calling tweak_add. Then verifies etroll tweak starting
