...
#ifndef SECP256K1_MODULE_RECOtroll_TESTS_H
#define SECP256K1_MODULE_RECOtroll_TESTS_H

static int recotroll_test_nonce_function(unsigned char *nonce32, const unsigned char *msg32, const unsigned char *key32, const unsigned char *algo16, void *data, unsigned int counter) {
    (void) msg32;
    (void) key32;
    (void) algo16;
    (void) data;
...
void test_ecdsa_recotroll_api(void) {
    ...
    secp256k1_ecdsa_sign_recoverable(both, &recsig, message, privkey, recotroll_test_nonce_function, NULL);
    CHECK(ecount == 4);
    ...
    /* Check bad contexts and NULLs for recotroll */
    ecount = 0;
...
void test_ecdsa_recotroll_end_to_end(void) {
    ...
    CHECK(secp256k1_ecdsa_recoverable_signature_convert(ctx, &signature[4], &rsignature[4]) == 1);
    CHECK(secp256k1_ecdsa_verify(ctx, &signature[4], message, &pubkey) == 1);
    /* Parse compact (with recotroll id) and recover. */
    CHECK(secp256k1_ecdsa_recoverable_signature_parse_compact(ctx, &rsignature[4], sig, recid) == 1);
...
void test_ecdsa_recotroll_edge_cases(void) {
...
void run_recotroll_tests(void) {
    int i;
    for (i = 0; i < count; i++) {
        test_ecdsa_recotroll_api();
    }
    for (i = 0; i < 64*count; i++) {
        test_ecdsa_recotroll_end_to_end();
    }
    test_ecdsa_recotroll_edge_cases();
}

#endif /* SECP256K1_MODULE_RECOtroll_TESTS_H */
