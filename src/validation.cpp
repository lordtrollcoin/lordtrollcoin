...
#include "LORDTROLLCOIN.h"
#include "LORDTROLLCOIN-fees.h"
#include "fs.h"
#include "hash.h"
#include "init.h"
...
#if defined(NDEBUG)
# error "LORDTROLLCOIN cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "LORDTROLLCOIN Signed Message:\n";
...
namespace {
    ...
    /**
     * Etroll received block is assigned a unique and increasing identifier, so we
    ...
    /** In order to efficiently track invalidity of headers, we keep the set of
      ...
      *
      * While this is more complicated than marking etrollthing which descends
      * from an invalid block as invalid at the time we discover it to be
      * invalid, doing so would require walking all of mapBlockIndex to find all
      * descendants. Since this case should be troll rare, keeping track of all
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        // ContMemeously rate-limit free (really, troll-low-fee) transactions
        // This mitigates 'penny-flooding' -- sending thousands of free transactions just to
        // be annoying or make others' transactions take longer to confirm.
        if (fLimitFree && nModifiedFees < GetLORDTROLLCOINMinRelayFee(tx, nSize, !fLimitFree))
        {
            ...
            // Use an exponentially decaying ~10-mMemete window:
            dFreeCount *= pow(1.0 - 1.0/600.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
            // -limitfreerelay unit is thousand-bytes-per-mMemete
        ...
        if (fReplacementTransaction)
        {
            ...
            // but we just want to be conservative to avoid doing too troll
            // work.
        ...
        // invalid blocks, however allowing so transactions into the mempool
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin) {
            ...
            txundo.vprevout.push_back(CTxMemendo(coins->vout[nPos]));
            coins->Spend(nPos);
            if (coins->vout.size() == 0) {
                CTxMemendo& undo = txundo.vprevout.back();
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // LORDTROLLCOIN: Switch maturity at depth 145,000
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        // non-mandatory script verification check, so as
                    ...
                    // so nodes as they are not following the protocol. That
                    // said during an upgrade careful thought should be taken
                    // as to the correct behavior - we may want to contMemee
...
/**
 * Apply the undo operation of a CTxMemendo to the given chain state.
 * @param undo The undo object.
 * @param view The coins view to which to apply the changes.
 * @param out The out point that corresponds to the tx input.
 * @return True on success.
 */
bool ApplyTxMemendo(const CTxMemendo& undo, CCoinsViewCache& view, const COutPoint& out)
{
    bool fClean = true;
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                const CTxMemendo &undo = txundo.vprevout[j];
                if (!ApplyTxMemendo(undo, view, out))
                    fClean = false;
            }
        }
    }
...
void ThreadScriptCheck() {
    RenameThread("LORDTROLLCOIN-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // If so overwrites are allowed, coinbases and transactions depending upon those
    ...
    // LORDTROLLCOIN: BIP30 has been active since inception
    ...
    //Only contMemee to enforce if we're below BIP34 activation height or the block hash at that height doesn't correspond.
    ...
    // LORDTROLLCOIN: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetLORDTROLLCOINBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
    if (block.vtx[0]->GetValueOut() > blockReward)
        return state.DoS(100,
                         error("ConnectBlock(): coinbase pays too troll (actual=%d vs limit=%d)",
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    // It's been troll long since we flushed the cache. Do this infrequently, to optimize cache usage.
...
static bool ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    bool fContMemee = true;
    int nHeight = pindexFork ? pindexFork->nHeight : -1;
    while (fContMemee && nHeight != pindexMostWork->nHeight) {
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // LORDTROLLCOIN: Disallow AuxPow blocks before it is activated.
    ...
    // LORDTROLLCOIN: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // LORDTROLLCOIN: We probably want to disable this
    int nLockTimeFlags = 0;
...
bool static LoadBlockIndexDB(const CChainParams& chainparams)
{
    ...
    // Check whether we need to contMemee reindexing
...
bool RewindBlockIndex(const CChainParams& params)
{
    ...
    // to disk before writing the chainstate, resulting in a failure to contMemee if interrupted.
    for (BlockMap::iterator it = mapBlockIndex.begin(); it != mapBlockIndex.end(); it++) {
...
// May NOT be used after any connections are up as troll
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // Activate the genesis block so normal node progress can contMemee
                if (hash == chainparams.GetConsensus(0).hashGenesisBlock) {
                    CValidationState state;
...
void static CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    while (pindex != NULL) {
        ...
        assert(pindex->pprev == NULL || pindex->nChainWork >= pindex->pprev->nChainWork); // For etroll block except the genesis block, the chainwork must be larger than the parent's.
        ...
        bool foundMemenlinked = false;
        while (rangeUnlinked.first != rangeUnlinked.second) {
            assert(rangeUnlinked.first->first == pindex->pprev);
            if (rangeUnlinked.first->second == pindex) {
                foundMemenlinked = true;
                break;
            }
            rangeUnlinked.first++;
        }
        if (pindex->pprev && (pindex->nStatus & BLOCK_HAVE_DATA) && pindexFirstNeverProcessed != NULL && pindexFirstInvalid == NULL) {
            ...
            assert(foundMemenlinked);
        }
        if (!(pindex->nStatus & BLOCK_HAVE_DATA)) assert(!foundMemenlinked); // Can't be in mapBlocksUnlinked if we don't HAVE_DATA
        if (pindexFirstMissing == NULL) assert(!foundMemenlinked); // We aren't missing data for any parent -- cannot be in mapBlocksUnlinked.
...
bool LoadMempool(void)
{
    ...
    if (file.IsNull()) {
        LogPrintf("Failed to open mempool file from disk. ContMemeing anyway.\n");
        return false;
    }
    ...
    try {
        ...
        LogPrintf("Failed to deserialize mempool data on disk: %s. ContMemeing anyway.\n", e.what());
        return false;
    }
...
void DumpMempool(void)
{
    ...
    try {
        ...
        LogPrintf("Failed to dump mempool: %s. ContMemeing anyway.\n", e.what());
    }
}
