...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s LORDTROLLCOIN-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  LORDTROLLCOIN-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded LORDTROLLCOIN transaction") + "\n" +
              "  LORDTROLLCOIN-tx [options] -create [commands]   " + _("Create hex-encoded LORDTROLLCOIN transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        PrintExceptionContMemee(NULL, "CommandLineRawTx()");
        throw;
    }
...
MAIN_FUNCTION
{
    ...
    try {
        ...
        PrintExceptionContMemee(&e, "AppInitRawTx()");
        return EXIT_FAILURE;
    } catch (...) {
        PrintExceptionContMemee(NULL, "AppInitRawTx()");
        return EXIT_FAILURE;
    }
    ...
    try {
        ...
        PrintExceptionContMemee(&e, "CommandLineRawTx()");
    } catch (...) {
        PrintExceptionContMemee(NULL, "CommandLineRawTx()");
    }
    return ret;
