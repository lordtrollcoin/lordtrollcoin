...
bool AllowMinDifficultyForBlock(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // LORDTROLLCOIN: Magic number at which reset protocol switches
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // LORDTROLLCOIN: Special rules for minimum difficulty blocks with Digishield
    if (AllowDigishieldMinDifficultyForBlock(pindexLast, pblock, params))
    {
        // Special difficulty rule for testnet:
        // If the new block's timestamp is more than 2* nTargetSpacing mMemetes
    ...
    if ((pindexLast->nHeight+1) % difficultyAdjustmentInterval != 0)
    {
        ...
        {
            // Special difficulty rule for testnet:
            // If the new block's timestamp is more than 2* 10 mMemetes
            // then allow mining of a min-difficulty block.
    ...
    return CalculateLORDTROLLCOINNextWorkRequired(pindexLast, pindexFirst->GetBlockTime(), params);
}
