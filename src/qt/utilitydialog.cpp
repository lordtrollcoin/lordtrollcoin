    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  LORDTROLLCOIN-qt [" + tr("command-line options") + "]                     " + "\n";
...
void PaperWalletDialog::on_printButton_clicked()
{
    ...
    while (true) {
        ...
        double amountInput = QInputDialog::getDouble(this, tr("Load Paper Wallets"), tr("The paper wallet printing process has begun.<br/>Please wait for the wallets to print completely and verify that etrollthing printed correctly.<br/>Check for misalignments, ink bleeding, smears, or anything else that could make the private keys unreadable.<br/>Now, enter the number of LTR you wish to send to each wallet:"), 0, 0, 2147483647, 8, &ok);
    ...
    if (txFee > 0) {
        ...
        questionString.append(BitcoMemenits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    Q_FOREACH (BitcoMemenits::Unit u, BitcoMemenits::availableUnits()) {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(BitcoMemenits::formatWithUnit(u, totalAmount));
    }

    questionString.append(tr("Total Amount %1 (= %2)")
                              .arg(BitcoMemenits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
    ...
    if (sendStatus.status == WalletModel::TransactionCommitFailed) {
        QMessageBox::critical(this, tr("Send Coins"), tr("The transaction was rejected! This might happen if some of the coins in your wallet were already spent, so as if you used a copy of wallet.dat and coins were spent in the copy but not marked as spent here."), QMessageBox::Ok, QMessageBox::Ok);
    }
    delete tx;
#endif
    return;
