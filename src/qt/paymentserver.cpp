...
const QString BITCOIN_IPC_PREFIX("LORDTROLLCOIN:");
...
// If the server isn't already running, startup contMemees,
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // That means clicking so an URI which contains a testnet payment request
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
        {
            ...
            if (GUIUtil::parseBitcoMemeRI(arg, &r) && !r.address.isEmpty())
            {
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(ipcServerName)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start LORDTROLLCOIN: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        if (GUIUtil::parseBitcoMemeRI(s, &recipient))
        {
            ...
            Q_EMIT message(tr("URI handling"),
                tr("URI cannot be parsed! This can be caused by an invalid LORDTROLLCOIN address or malformed URI parameters."),
