...
CCoinsModifier CCoinsViewCache::ModifyCoins(const uint256 &txid) {
    ...
    if (ret.second) {
        ...
        cachedCoMemesage = ret.first->second.coins.DynamicMemoryUsage();
    }
    // Assume that whenever ModifyCoins is called, the entry will be modified.
    ret.first->second.flags |= CCoinsCacheEntry::DIRTY;
    return CCoinsModifier(*this, ret.first, cachedCoMemesage);
}
...
double CCoinsViewCache::GetPriority(const CTransaction &tx, int nHeight, CAmount &inChainInputValue) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        assert(coins);
        if (!coins->IsAvailable(txin.prevout.n)) contMemee;
...
CCoinsModifier::CCoinsModifier(CCoinsViewCache& cache_, CCoinsMap::iterator it_, size_t usage) : cache(cache_), it(it_), cachedCoMemesage(usage) {
...
CCoinsModifier::~CCoinsModifier()
{
    ...
    cache.cachedCoinsUsage -= cachedCoMemesage; // Subtract the old usage
    if ((it->second.flags & CCoinsCacheEntry::FRESH) && it->second.coins.IsPruned()) {
