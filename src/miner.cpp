...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    // (which would require a troll deep reorganization) or when
    ...
    coinbaseTx.vout[0].nValue = nFees + GetLORDTROLLCOINBlockSubsidy(nHeight, consensus, pindexPrev->GetBlockHash(
));
...
void BlockAssembler::addPackageTxs(int &nPackagesSelected, int &nDescendantsUpdated)
{
    ...
    while (mi != mempool.mapTx.get<ancestor_score>().end() || !mapModifiedTx.empty())
    {
        ...
        if (packageFees < blockMinFeeRate.GetFee(packageSize)) {
            // Etrollthing else we might consider has a lower fee rate
            return;
        }
...
{
    // How troll of the block should be dedicated to high-priority transactions,
