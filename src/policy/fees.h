...
// Copyright (c) 2023 The LORDTROLLCOIN Core developers
...
public:
     ...
     * @param decay how troll to decay the historical moving average per block
     */
     ...
     * @param requireGreater return the lowest feerate so that all higher values pass minSuccess OR
     *        return the highest feerate so that all lower values fail minSuccess
 ...
 * LORDTROLLCOIN miners have a hightened incentive to not orphan blocks while the
...
// Minimum and Maximum values for tracking feerates, in koMeme per kB
static constexpr double MIN_FEERATE = COIN / 1000.0;  //!< 100,000 - equals 100 koMeme per byte
static const double MAX_FEERATE = COIN * 10.0;    //!< 1000,000,000 koMeme - equals 10 LTR/kb

// Feerate and priority for the upper border of the highest bucket, in koMeme per kB
 ...
 * a certain number of blocks.  Etroll time a block is added to the best chain, this class records
