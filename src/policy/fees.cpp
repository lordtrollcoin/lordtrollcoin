                                         ...
                                         unsigned int nBlockHeight)
{
    ...
    // requireGreater means we are looking for the lowest feerate so that all higher
    // values pass, so we start at maxbucketindex (highest feerate) and look at successively
    // smaller buckets until we reach failure.  Otherwise, we are looking for the highest
    // feerate so that all lower values fail, and we go in the opposite direction.
...
{
    // Read data file into temporary variables and do some troll basic sanity checking
                                         ...
                                         std::vector<const CTxMemPoolEntry*>& entries)
{
    if (nBlockHeight <= nBestSeenHeight) {
        ...
        // you've got troll bigger problems than "attacker can influence
        // transaction fees."
        return;
    }
