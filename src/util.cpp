...
const char * const BITCOIN_CONF_FILENAME = "LORDTROLLCOIN.conf";
const char * const BITCOIN_PID_FILENAME = "LORDTROLLCOINd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        // if not debugging etrollthing and not debugging specific category, LogPrint does nothing.
...
#else
    const char* pszModule = "LORDTROLLCOIN";
#endif
    if (pex)
        return strprintf(
...
void PrintExceptionContMemee(const std::exception* pex, const char* pszThread)
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LORDTROLLCOIN";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LORDTROLLCOIN";
#else
    // Unix
    return pathRet / ".LORDTROLLCOIN";
#endif
#endif
}
...
#else
    #if defined(__lMemex__) || defined(__NetBSD__)
...
#endif
    // On most POSIX systems (e.g. LMemex, but not BSD) the environment's locale
