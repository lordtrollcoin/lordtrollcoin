...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample)
{
    ...
    LogPrint("net","added time data, samples %d, offset %+d (%+d mMemetes)\n", vTimeOffsets.size(), nOffsetSample, nOffsetSample/60);
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        // Only let other nodes change our time by so troll
        ...
        else
        {
            ...
            if (!fDone)
            {
                // If nobody has a time different than ours but within 5 mMemetes of ours, give a warning
                bool fMatch = false;
        ...
        LogPrint("net", "nTimeOffset = %+d  (%+d mMemetes)\n", nTimeOffset, nTimeOffset/60);
    }
}
