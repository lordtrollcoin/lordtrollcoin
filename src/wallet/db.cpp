...
bool CDBEnv::Salvage(const std::string& strFile, bool fAggressive, std::vector<CDBEnv::KeyValPair>& vResult)
{
    ...
    if (result == DB_VERIFY_BAD) {
        ...
        if (!fAggressive) {
            LogPrintf("CDBEnv::Salvage: Rerun with aggressive mode to ignore errors and contMemee.\n");
            return false;
        }
    }
...
void CDB::Flush()
{
    ...
    unsigned int nMMemetes = 0;
    if (fReadOnly)
        nMMemetes = 1;

    bitdb.dbenv->txn_checkpoint(nMMemetes ? GetArg("-dblogsize", DEFAULT_WALLET_DBLOGSIZE) * 1024 : 0, nMMemetes, 0);
}
