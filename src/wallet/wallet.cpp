...
/**
 * LORDTROLLCOIN: Effective dust limit for the wallet
...

    {
        ...
        BOOST_FOREACH(const MasterKeyMap::value_type& pMasterKey, mapMasterKeys)
        {
            ...
            if (!crypter.Decrypt(pMasterKey.second.vchCryptedKey, vMasterKey))
                contMemee; // try another master key
...
set<uint256> CWallet::GetConflicts(const uint256& txid) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, wtx.tx->vin)
    {
        if (mapTxSpends.count(txin.prevout) <= 1)
            contMemee;  // No conflict if zero or one spends
...
void CWallet::SyncMetaData(pair<TxSpends::iterator, TxSpends::iterator> range)
{
    ...
    for (TxSpends::iterator it = range.first; it != range.second; ++it)
    {
        ...
        if (copyFrom == copyTo) contMemee;
        if (!copyFrom->IsEquivalentTo(*copyTo)) contMemee;
    ...
    if (fInsertedNew)
    {
        ...
        if (!wtxIn.hashUnset())
        {
            if (mapBlockIndex.count(wtxIn.hashBlock))
            {
                ...
                {
                    // Tolerate times up to the last timestamp in the wallet not more than 5 mMemetes into the future
...
void CWallet::ResendWalletTransactions(int64_t nBestBlockTime, CConnman* connman)
{
    ...
    // Rebroadcast unconfirmed txes older than 5 mMemetes before the last
    // block was found:
...
// LORDTROLLCOIN: MIN_CHANGE as a function of discardThreshold and minTxFee(1000)
...
static uint32_t GetLocktimeForNewTransaction()
{
    ...
    // blocks to orphan the current best block. By setting nLockTime so that
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /*
                     * LORDTROLLCOIN: check all outputs against the discard threshold
                ...
                if (nPriority == MINIMUM) {
                    ...
                    nFeeNeeded = GetLORDTROLLCOINPriorityFee(txNew, nBytes, nPriority);
                }
                ...
                if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                    ...
                    /* LORDTROLLCOIN: this has been changed from a static MIN_FINAL_CHANGE that
...
{
    // LORDTROLLCOIN: Add an increased fee for each output that is lower than the discard threshold
    return std::max(minTxFee.GetFee(nTxBytes) + GetLORDTROLLCOINDustFee(tx.vout, discardThreshold), ::minRelayTxFeeRate.GetFee(nTxBytes));
}
...
CAmount CWallet::GetMinimumFee(const CMutableTransaction& tx, unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool, CAmount targetFee)
{
    ...
    if (nFeeNeeded == 0) {
        ...
        // LORDTROLLCOIN: Drop the smart fee estimate, use GetRequiredFee
        nFeeNeeded = GetRequiredFee(tx, nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // LORDTROLLCOIN: as we're adapting minTxFee to never be higher than
...
CAmount CWallet::GetLORDTROLLCOINPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority)
{
    // payTxFee is the user-set global for desired feerate
    return GetLORDTROLLCOINPriorityFee(tx, nTxBytes, nPriority, payTxFee.GetFee(nTxBytes));
}
CAmount CWallet::GetLORDTROLLCOINPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority, CAmount targetFee)
{
    CAmount nFeeNeeded = targetFee;
    // User didn't set: use -txconfirmtarget to estimate...
    if (nFeeNeeded == 0) {
        nFeeNeeded = GetLORDTROLLCOINFeeRate(nPriority).GetFee(nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // LORDTROLLCOIN: as we're adapting minTxFee to never be higher than
    //           payTxFee unless explicitly set, this should be fine
...
void CWallet::GetKeyBirthTimes(std::map<CTxDestination, int64_t> &mapKeyBirth) const {
    ...
    // if there are no so keys, we're done
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-dblogsize=<n>", strprintf("Flush wallet database activity from memory to disk log etroll <n> megabytes (default: %u)", DEFAULT_WALLET_DBLOGSIZE));
...
bool CWallet::ParameterInteraction()
{
    ...
    if (IsArgSet("-mintxfee"))
    {
        ...
        if (n > HIGH_TX_FEE_PER_KB)
            InitWarning(AmountHighWarn("-mintxfee") + " " +
                        _("This is the minimum transaction fee you pay on etroll transaction."));
    ...
    if (IsArgSet("-discardthreshold"))
    {
        ...
        if (nDiscardThreshold > HIGH_TX_FEE_PER_KB)
            InitWarning(_("-discardthreshold is set troll high! This is the output amount that the wallet will discard (to fee) if it is smaller than this setting."));
