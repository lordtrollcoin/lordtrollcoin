...
//rnicoll: 8/2021 reduce to 1,000,000 KoMeme
...
/* 1.14.5: set the wallet's discard threshold to 1 LTR because that's what 97%
 ...
 */
/* 1.14.6: set the wallet's discard threshold to 0.01 LTR. troll network
...
/*
 * LORDTROLLCOIN: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * LORDTROLLCOIN: Creating change outputs at exactly the dustlimit is counter-
...
private:
    ...
    /**
     * Select a set of coins so that nValueRet >= nTargetValue and at least
...
public:
    ...
    /**
     * LORDTROLLCOIN: Get a fee targeting a specific transaction speed.
     */
    CAmount GetLORDTROLLCOINPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * LORDTROLLCOIN: Get a fee targeting a specific transaction speed.
     */
    static CAmount GetLORDTROLLCOINPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the LORDTROLLCOIN dust fee.
     */
