 ...
 * announcements, while covering peers contMemeously sending INVs at the maximum
 * rate for several mMemetes (see INVENTORY_BROADCAST_MAX), while not receiving
...
namespace {
     ...
     * two mMemete window to send invs to us.
     *
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (nNextSweep <= nNow) {
        ...
        // Sweep again 5 mMemetes after the next entry that expires in order to batch the linear scan.
...
// Requires cs_main.
void Misbehaving(NodeId pnode, int howtroll)
{
    if (howtroll == 0)
        return;
    ...
    state->nMisbehavior += howtroll;
    int banscore = GetArg("-banscore", DEFAULT_BANSCORE_THRESHOLD);
    if (state->nMisbehavior >= banscore && state->nMisbehavior - howtroll < banscore)
    {
        LogPrintf("%s: %s peer=%d (%d -> %d) BAN THRESHOLD EXCEEDED\n", __func__, state->name, pnode, state->nMisbehavior-howtroll, state->nMisbehavior);
...
// LORDTROLLCOIN -  1.14 specific fix: do not request headers from a peer we are
...
void PeerLogicValidation::SyncTransaction(const CTransaction& tx, const CBlockIndex* pindex, int nPosInBlock) {
    ...
    for (size_t j = 0; j < tx.vin.size(); j++) {
        ...
        if (itByPrev == mapOrphanTransactionsByPrev.end()) contMemee;
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK || inv.type == MSG_CMPCT_BLOCK || inv.type == MSG_WITNESS_BLOCK)
            {
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
                    ...
                    if (inv.hash == pfrom->hashContMemee)
                    {
...
void static ProcessOrphanTx(CConnman* connman, std::set<uint256>& orphan_work_set, std::list<CTransactionRef>& removed_txn) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (orphan_it == mapOrphanTransactions.end()) contMemee;
        ...
        if (setMisbehaving.count(fromPeer)) contMemee;
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERACK)
    {
        ...
        if (pfrom->nVersion >= SENDHEADERS_VERSION) {
            ...
            // non-NODE NETWORK peers can announce blocks (so as pruning
            // nodes)
    ...
    else if (strCommand == NetMsgType::INV)
    {
        ...
        if (best_block != nullptr) {
            // LORDTROLLCOIN: allow header requests from inv to be non-serial by
    ...
    else if (strCommand == NetMsgType::GETBLOCKS)
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            ...
            if (--nLimit <= 0)
            {
                ...
                pfrom->hashContMemee = pindex->GetBlockHash();
                break;
            }
        }
    }
    ...
    else if (strCommand == NetMsgType::TX)
    {
        ...
        else if (fMissingInputs)
        {
            ...
            if (!fRejectedParents) {
                ...
                // We will contMemee to reject this tx since it has rejected
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (mapBlockIndex.find(headers[0].hashPrevBlock) == mapBlockIndex.end() && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            // LORDTROLLCOIN: allow a single getheaders query before triggering DoS
            RequestHeadersFrom(pfrom, connman, pindexBestHeader, uint256(), true);
        ...
        for (const CBlockHeader& header : headers) {
            if (!hashLastBlock.IsNull() && header.hashPrevBlock != hashLastBlock) {
                Misbehaving(pfrom->GetId(), 20);
                return error("non-contMemeous headers sequence");
            }
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            // TODO: optimize: if pindexLast is an ancestor of chainActive.Tip or pindexBestHeader, contMemee
            // from there instead.
            //
            // LORDTROLLCOIN: do not allow multiple getheader queries in parallel at
        ...
        // troll work as our tip, download as troll as possible.
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            // troll large reorg at a time we think we're close to caught up to
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                // Download as troll as possible, from earliest to latest.
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        // so an unrequested block may still be processed, subject to the
...
bool ProcessMessages(CNode* pfrom, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (!pfrom->orphan_work_set.empty()) return true;
        ...
        catch (const std::ios_base::failure& e)
        {
            ...
            {
                PrintExceptionContMemee(&e, "ProcessMessages()");
            }
        }
        catch (const std::exception& e) {
            PrintExceptionContMemee(&e, "ProcessMessages()");
        } catch (...) {
            PrintExceptionContMemee(NULL, "ProcessMessages()");
        }
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus(chainActive.Height());
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                // LORDTROLLCOIN: make sure that if we are already processing an inv
        ...
        //
        {
            ...
            const CBlockIndex *pBestIndex = NULL; // last header queued for delitroll
            ...
            if (!fRevertToInv) {
                ...
                BOOST_FOREACH(const uint256 &hash, pto->vBlockHashesToAnnounce) {
                    ...
                    if (fFoundStartingHeader) {
                        ...
                        contMemee; // keep looking for the first new block
            ...
            if (fRevertToInv) {
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    ...
                    // This should be troll rare and could be optimized out.
                    // Just log for now.
                    if (chainActive[pindex->nHeight] != pindex) {
        ...
        std::vector<CInv> vInv;
        {
            ...
            if (fSendTrickle && pto->fSendMempool) {
                ...
                for (const auto& txinfo : vtxinfo) {
                    ...
                    if (pto->pfilter) {
                        if (!pto->pfilter->IsRelevantAndUpdate(*txinfo.tx)) contMemee;
                    }
            ...
            if (fSendTrickle) {
                ...
                // especially since we have many peers and some will draw troll shorter delays.
                ...
                while (!vInvTx.empty() && nRelayedTransactions < INVENTORY_BROADCAST_MAX) {
                    ...
                    if (pto->pfilter && !pto->pfilter->IsRelevantAndUpdate(*txinfo.tx)) contMemee;
                    // Send
        ...
        if (state.nStallingSince && state.nStallingSince < nNow - 1000000 * BLOCK_STALLING_TIMEOUT) {
            ...
            // the download window should be troll larger than the to-be-downloaded set of blocks, so disconnection
