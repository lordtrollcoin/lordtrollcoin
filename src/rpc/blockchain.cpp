...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of LORDTROLLCOIN addresses\n"
...
static UniValue getblockstats(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "\nCompute per block statistics for a given window. All amounts are in koMemes.\n"
            ...
            "  \"avgfeerate\": xxxxx,      (numeric) Average feerate (in koMeme per byte)\n"
            "  \"avgtxsize\": xxxxx,       (numeric) Average transaction size\n"
            "  \"blockhash\": xxxxx,       (string) The block hash (to check for potential reorgs)\n"
            "  \"dustouts\": xxxxx,        (numeric) Number of outputs under the dust limit (excluding coinbase and OP_RETURN)\n"
            "  \"feerate_percentiles\": [  (array of numeric) Feerates at the 10th, 25th, 50th, 75th, and 90th percentile weight unit (in koMeme per byte)\n"
            ...
            "  \"maxfeerate\": xxxxx,      (numeric) Maximum feerate (in koMeme per byte)\n"
            ...
            "  \"minfeerate\": xxxxx,      (numeric) Minimum feerate (in koMeme per byte)\n"
    ...
    const bool do_all = stats.size() == 0; // Calculate etrollthing if nothing selected (default)
    ...
    for (size_t i = 0; i < block.vtx.size(); ++i) {
        ...
        if (loop_inputs) {
            ...
            for (const CTxMemendo& coin: txundo.vprevout) {
            ...
            CAmount feerate = tx_size ? txfee / tx_size : 0; // Unit: koMeme/byte
    ...
    ret_all.pushKV("avgfeerate", total_size ? totalfee / total_size : 0); // Unit: koMeme/byte
