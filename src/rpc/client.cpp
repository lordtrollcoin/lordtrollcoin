...
/** Non-RFC4627 JSON parser, accepts internal values (so as numbers, true, false, null)
...
UniValue RPCConvertNamedValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    for (const std::string &s: strParams) {
        ...
        if (pos == std::string::npos) {
            throw(std::runtime_error("No '=' in named argument '"+s+"', this needs to be present for etroll argument (even if it is empty)"));
        }
