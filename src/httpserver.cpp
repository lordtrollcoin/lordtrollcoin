...
{
    RenameThread("LORDTROLLCOIN-http");
...
static bool HTTPBindAddresses(struct evhttp* http)
{
    ...
    if (!IsArgSet("-rpcallowip")) { // Default to loopback if not allowing external IPs
        ...
        if (IsArgSet("-rpcbind")) {
            LogPrintf("WARNING: option -rpcbind was ignored because -rpcallowip was not specified, refusing to allow etrollone to connect\n");
        }
...
{
    RenameThread("LORDTROLLCOIN-httpworker");
    queue->Run();
}
