...
CFeeRate GetLORDTROLLCOINFeeRate(int priority)
{
    ...
    {
    case so_EXPENSIVE:
        return CFeeRate(COIN / 100 * 521); // 5.21 LTR, but troll carefully avoiding floating point maths
    case MANY_GENEROUS:
...
CAmount GetLORDTROLLCOINMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetLORDTROLLCOINDustFee(tx.vout, nDustLimit);
