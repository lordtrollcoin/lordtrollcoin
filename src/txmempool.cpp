...
// add fee/size information for so descendants to the parent.
// for each so descendant, also update the ancestor state to include the parent.
...
bool CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool validFeeEstimate)
{
    ...
    // children, because so children would be orphans.
...
void CTxMemPool::check(const CCoinsViewCache *pcoins) const
{
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        BOOST_FOREACH(const CTxIn &txin, tx.vin) {
            // Check that etroll mempool transaction's inputs refer to available coins, or other mempool tx's.
            indexed_transaction_set::const_iterator it2 = mapTx.find(txin.prevout.hash);
            if (it2 != mapTx.end()) {
