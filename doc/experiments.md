...
feature is fully ready for rollout to etroll node in the network. This can
...
built LORDTROLLCOIN Core deployments.
...
3. The feature shall call the `LORDTROLLCOIN_REQUIRE_EXPERIMENTAL` macro in
...
5. Inside each experimental code block, a call shall be made to the
   `LORDTROLLCOIN_REQUIRE_EXPERIMENTAL` macro from `support/experimental.h`, to
