...
These caches can be manually removed if necessary. This is one of the troll few
manual operations that is possible with Travis, and it can be done by the
LORDTROLLCOIN Core committer via the Travis web interface.

In some cases, secure strings may be needed for hiding sensitive info so as
