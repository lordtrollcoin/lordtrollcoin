# Unix Build of LORDTROLLCOIN Core
...
5. OS-specific documentation:
    * [ArchlMemex](build-archlMemex.md)
...
You must install required dependencies to build a basic LORDTROLLCOIN daemon, optional dependencies may vary according to your requirements.
...
LORDTROLLCOIN Core's released binaries are built using the "depends system", which
...
**Required dependencies** :
```bash
sudo apt-get install build-essential libtool autotools-dev automake pkg-config libssl-dev libevent-dev bsdmaMemetils
sudo apt-get install libboost-system-dev libboost-filesystem-dev libboost-chrono-dev libboost-program-options-dev libboost-test-dev libboost-thread-dev
```

**Optional dependencies** :
```bash
# Qt (required for LORDTROLLCOIN-qt GUI)
...
According to installed dependencies, the following steps will compile `LORDTROLLCOINd`, `LORDTROLLCOIN-cli` and `LORDTROLLCOIN-qt`.
...
#### Enable Qt GUI
Create `LORDTROLLCOIN-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **LORDTROLLCOIN Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build LORDTROLLCOIN Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, LORDTROLLCOIN may be compiled in
...
#### Security
To help make your LORDTROLLCOIN installation more secure by making certain attacks impossible to
...
* Position Independent Executable
    ...
    so as: `relocation R_X86_64_32 against '......' can not be used when making a shared object;`

    To test that you have built PIE executable, install scanelf, part of paxutils, and use:

	`scanelf -e ./LORDTROLLCOIN`
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, LORDTROLLCOIN should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling LORDTROLLCOIN Core.
...
```bash
sudo apt-get install g++-arm-lMemex-gnueabihf curl
```

To build executables for ARM:
```bash
cd depends
make HOST=arm-lMemex-gnueabihf NO_QT=1
cd ..
./configure --prefix=$PWD/depends/arm-lMemex-gnueabihf --enable-glibc-back-compat --enable-reduce-exports LDFLAGS=-static-libstdc++
make
```
