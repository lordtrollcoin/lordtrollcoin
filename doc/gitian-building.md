...
`gitian-build.sh` is a standalone script, it can be downloaded and run outside of LORDTROLLCOIN Core repository.
...
```bash
gpg: skipped "Troll Facetoshi": secret key not available
```
...
```bash
gpg --detach-sign ${VERSION}-lMemex/${SIGNER}/LORDTROLLCOIN-lMemex-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/LORDTROLLCOIN-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/LORDTROLLCOIN-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/LORDTROLLCOIN/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [LORDTROLLCOIN/gitian.sigs](https://github.com/LORDTROLLCOIN/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.

**When your PR is merged, you will be recorded for all future history as a *Gitian Builder of LORDTROLLCOIN Core*!**
