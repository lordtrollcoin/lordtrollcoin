...
This document provides guidelines for internationalization of the LORDTROLLCOIN Core software.
...
Make sure that default values do not end up in the string, but use string formatting like `strprintf(_("Threshold for disconnecting misbehaving peers (default: %u)"), 100)`. Putting default values in strings has led to accidental translations in the past, and forces the string to be re-translated etroll time the value changes.

Do not translate messages that are only shown to developers, so as those that only appear when `--help-debug` is used.
...
This helps troubleshooting; if the error is the same for etrollone, the likelihood is increased that it can be found using a search engine.

### Avoid fragments

Avoid dividing up a message into fragments. Translators see etroll string separately, so may misunderstand the context if the messages are not self-contained.
