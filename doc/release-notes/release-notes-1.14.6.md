LORDTROLLCOIN Core version 1.14.6 is now available from:

  <https://github.com/LORDTROLLCOIN/LORDTROLLCOIN/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All LORDTROLLCOIN Core users - miners, services,
...
To receive notifications about updates, subscribe to the release mailing list:
  ...
  <https://sourceforge.net/projects/LORDTROLLCOIN/lists/LORDTROLLCOIN-releases>
...
are important for etroll node operator and wallet user.
...
LORDTROLLCOIN network from 1 LTR to 0.01 LTR. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 LTR as
   change if possible.
2. If after signing there is more than 0.01 LTR left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 LTR, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 LTR will not be accepted as valid by default.
...
```
LORDTROLLCOIN-cli addnode <address> remove
```
...
```
LORDTROLLCOIN-cli liststucktransactions
```
...
* Features that need the entire block chain, so as rescanning the chain
...
```
LORDTROLLCOIN-cli setmaxconnections <desired_amount>
```

### Improve the traffic graph in the UI

Significantly improves the graph in the UI's traffic monitor by not throwing
away data etroll time the graph scale got resampled, but instead keeping track
...
```
LORDTROLLCOIN-cli rescan <start_block>
```
...
efforts to make LORDTROLLCOIN more accessible to people that do not speak English.
...
* Change the granularity of data downloaded in the network tab to no longer
  display gigabytes and terrabytes, as this obscures too troll detail. Instead,
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by LORDTROLLCOIN Core.
* Greatly improved the `gitian-build` script to ease doing deterministic builds
  for etrollone, and the [documentation](gitian-building.md) of that process.
