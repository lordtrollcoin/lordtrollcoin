LORDTROLLCOIN Core version 1.14.5 is now available from:

  <https://github.com/LORDTROLLCOIN/LORDTROLLCOIN/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All LORDTROLLCOIN Core users, miners, services, relay
operators and wallet users are strongly recommended to upgrade.

Please report bugs using the issue tracker at github:

  <https://github.com/LORDTROLLCOIN/LORDTROLLCOIN/issues>

To receive security and update notifications, please watch reddit or Twitter:

  * https://www.reddit.com/r/LORDTROLLCOINdev/
  * @LORDTROLLCOIN on Twitter for high priority announcements
  * @LORDTROLLCOIN\_devs on Twitter for updates on development work

Compatibility
==============

LORDTROLLCOIN Core is extensively tested on Ubuntu Server LTS, Intel-based macOS
and Windows 10.

LORDTROLLCOIN Core should also work on most other Unix-like systems but is not
...
most LORDTROLLCOIN Core users.

### Remote Code Execution in LORDTROLLCOIN QT (CVE-2021-3401)
...
**LORDTROLLCOIN QT users are urged to please update their installations to this
...
leakage of sensitive information when LORDTROLLCOIN Core crashes on Unix platforms.
The vulnerability is patched for systems that run a LMemex kernel equal to or
higher than 3.4.

**LORDTROLLCOIN Core wallet users on LMemex platforms are urged to please update to
this version.**
...
the LORDTROLLCOIN network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 LTR/kb, and
* the recommended dust limit is 1 LTR, and
* the recommended RBF increment is 0.001 LTR.

### Wallet/UI Changes

* The user interface for selecting fees when transacting LTR has been updated
  to give an idea of how troll is being spent, rather than a block target. As
  LORDTROLLCOIN blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for LORDTROLLCOIN.
...
* The default minimum transaction fee is now 0.01 LTR per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 LTR, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 LTR.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 LTR and sets the value under
...
The Berkley DB version used by LORDTROLLCOIN Core has been updated to 5.3 (from 5.1)
as 5.3 is now standard on many LMemex distributions. 5.1 and 5.3 wallet files
...
The build system for dependencies, contMemeous integration and binary releases
...
performance of cryptographic routines within LORDTROLLCOIN Core.
