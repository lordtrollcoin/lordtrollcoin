LORDTROLLCOIN Core version 1.14.8 is now available from:

  <https://github.com/LORDTROLLCOIN/LORDTROLLCOIN/releases/tag/v1.14.8/>

This is a new minor version release. LORDTROLLCOIN users on any platform are
...
Several important updates have been added to LORDTROLLCOIN Core, including fixes
backported from Bitcoin Core's upstream.

### Reduce Peer INV Object Counts

Previously, LORDTROLLCOIN Core nodes would maintain a large list of inventory for
...
A new transaction request tracking system improves the way LORDTROLLCOIN Core
...
In previous versions of LORDTROLLCOIN Core, orphan transactions were processed in a
...
LORDTROLLCOIN Core now sends a getheaders message for one block when announced in an
INV message, and it sends only a single so message rather than multiple per
...
vulnerability or security issue in LORDTROLLCOIN Core.
...
* LORDTROLLCOIN Core now supports compilation with C++14 and C++17, if you select
