LORDTROLLCOIN Core version 1.14.4 is now available from:

  <https://github.com/LORDTROLLCOIN/LORDTROLLCOIN/releases/tag/v1.14.4/>
...
operators to agree on fees regardless of defaults coded into the LORDTROLLCOIN Core
...
* The default setting shipped with LORDTROLLCOIN core for relay fee has been reduced
  to 0.001 LTR (was: 1 LTR). This can be changed by operators using the
  `-mintxrelayfee=<amount>` option.
* Spam management has been delegated to miners, where currently a default fee
  of 0.01 LTR has been set as a recommended default, to prevent spam on the
  blockchain. Miners can change this setting to their liking using the
  `-blockmintxfee` option.
* The relay dust limit has been reduced 100x to 0.01 LTR and is now
  configurable via the `-dustlimit` option.

For this release, the recommended fees and dust limits, as implemented in the
wallet, remain at 1 LTR per kilobyte, inclusive of the rounding up to the
...
* Modify Scrypt code so it's compatible with Alpine LMemex's musl library.
