LORDTROLLCOIN Core version 1.14.7 is now available from:

  <https://github.com/LORDTROLLCOIN/LORDTROLLCOIN/releases/tag/v1.14.7/>

This is a new minor version release, including enchancements to several RPC
methods and important security updates for LORDTROLLCOIN-Qt. LORDTROLLCOIN-Qt users on
...
LORDTROLLCOIN Core is extensively tested on Ubuntu Server LTS, macOS and Windows.
...
This release contains fixes for LORDTROLLCOIN Qt across all platforms that increase
security for end users.

### Disable BIP-70 payment server by default

To mitigate future potential risk inside LORDTROLLCOIN Qt, support for BIP-70 payment
...
inside LORDTROLLCOIN Qt. By default, incoming BIP-70 payment requests and files or
...
LORDTROLLCOIN.conf or as an argument to LORDTROLLCOIN-Qt if absolutely needed. Please use
extreme caution when exercising this option.

This is step 1 in full deprecation of BIP-70, BIP-71 and BIP-72 inside LORDTROLLCOIN
...
* Updated the `rpcuser.py` script to improve the password and salt generation
  ...
  current authentication in `LORDTROLLCOIN.conf` with new credentials generated with
...
Fee estimation was explicitly not ported or supported on LORDTROLLCOIN Core since
...
to support LORDTROLLCOIN parametrization.
...
```
LORDTROLLCOIN-cli help getblock
```
...
```
LORDTROLLCOIN-cli help getblockstats
```
...
* Fixed the example `LORDTROLLCOINd.service` file (#3066)
* Fixed a bug where misnamed signal handlers were causing excessive warnings
  in LORDTROLLCOIN Qt logs (#3063)
...
* Enhanced the CI to run unit tests in armhf and aarch64 lMemex builds (#3025)
...
* Updated the python `ltc-scrypt` module to a maintained fork (#3080), which
  can be found at https://github.com/LORDTROLLCOIN/ltc-scrypt
