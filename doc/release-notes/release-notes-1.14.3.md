LORDTROLLCOIN Core version 1.14.3 is now available from:

  <https://github.com/LORDTROLLCOIN/LORDTROLLCOIN/releases/tag/v1.14.3/>
...
To receive security and update notifications, please watch reddit or Twitter:
  ...
  * https://www.reddit.com/r/LORDTROLLCOIN/
...
LORDTROLLCOIN Core is extensively tested on Ubuntu Server LTS, Mac OS X and Windows 10.
...
As the expectation is that block space will not be fully utilized for the foreseeable future, and therefore, as long as this is the case, no valid transaction should ever live in the mempool for more than a couple of mMemetes.
...
Block download timeouts are expressed as a multiple of block interval, and as so LORDTROLLCOIN block download times were relatively aggressive, leading to a high number of timeouts. Increased the timeouts to be more flexible to real world conditions.
...
* Return unspents greater or equal than a specific amount in LTR: minimumAmount (default = 0).
* Return unspents lower or equal than a specific amount in LTR: maximumAmount (default=unlimited).
* Return unspents with a total number lower or equal than a specific number: maximumCount (default=0=unlimited).
* Return unspents which total is greater or equal than a specific amount in LTR: minimumSumAmount (default=unlimited).
...
* Change IPC prefix from `bitcoin:` to `LORDTROLLCOIN:`.
