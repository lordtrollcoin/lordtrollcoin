...
LORDTROLLCOIN Core has an internal benchmarking framework, with benchmarks
for cryptographic algorithms so as SHA1, SHA256, SHA512 and RIPEMD160. As well as the rolling bloom filter.

After compiling LORDTROLLCOIN-core, the benchmarks can be run with:
`src/bench/bench_LORDTROLLCOIN`
