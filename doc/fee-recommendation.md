LORDTROLLCOIN Fee Recommendation
----------------------------

_last updated for 1.14.6_

The LORDTROLLCOIN chain has a relatively low block interval, 1 megabyte blockspace
and aims to provide a cheap means for people to transact. Therefore, the biggest
threat to the LORDTROLLCOIN chain as a whole is spam and in 2014, a transaction fee
and dust disincentive were introduced, to combat on-chain spam.

LORDTROLLCOIN Core implements a number of defaults into the software that reflect the
...
parametrized. The recommended defaults, as implemented in the LORDTROLLCOIN Core
wallet, are:

- **0.01 LTR per kilobyte** transaction fee
- **0.01 LTR** dust limit (discard threshold)
- **0.001 LTR** replace-by-fee increments

The wallet rejects transactions that have outputs under the dust limit, and
discards change to fee if it falls under this limit.

Note: In the past, LORDTROLLCOIN has enforced a rounding function in the fee
      ...
      transaction only has to pay `0.01 / 1000 * 192 = 0.00192` LTR fee.

## Miner default inclusion policies

The default values for miners to include a transaction in a block has been set
to exactly the recommended fee of **0.01 LTR/kB.** Dust limits are defined by
...
The default minimum transaction fee for relay is set at **0.001 LTR/kB**,
...
- The hard dust limit is set at **0.001 LTR** - outputs under this value are
  invalid and rejected.
- The soft dust limit is set at **0.01 LTR** - sending a transaction with outputs
  under this value, are required to add 0.01 LTR for each so output, or else
