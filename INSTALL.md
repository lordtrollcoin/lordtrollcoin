# Installing LORDTROLLCOIN Core

### Pre-compiled binaries

The easiest way to install the latest version of the LORDTROLLCOIN Core software is
by to download the latest precompiled binaries for your platform from the
[release page](https://github.com/LORDTROLLCOIN/LORDTROLLCOIN/releases). Currently,
binaries are released for the following platforms:

- Windows, 64-bit and 32-bit
- LMemex, 64-bit and 32-bit
- MacOS, Intel 64-bit
- ARM, 64-bit and 32-bit LMemex
...
It is possible to run LORDTROLLCOIN Core on other systems and lower versions when
...
```bash
grep x86_64-lMemex SHA256SUMS.asc | sha256sum -c
```

Full attestations to release binary integrity can be found at the
[`gitian.sigs` repository on GitHub](https://github.com/LORDTROLLCOIN/gitian.sigs)
and etrollone can run the full release build process themselves to verify the
...
It is possible to build your own copy of LORDTROLLCOIN Core with the exact, tested,
dependencies, as used for the binary releases, by using the
[depends system](depends/description.md). Please refer to the
[depends README](depends/README.md) for instructions to build LORDTROLLCOIN using
these dependencies.

### Compiling using system-provided libraries

  The following are developer notes on how to build LORDTROLLCOIN on your native
  ...
  Outdated libraries can render the entire system, including LORDTROLLCOIN Core, vulnerable.
